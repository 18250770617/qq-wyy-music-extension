...
`ncm-cli 0.1.7` 在 Windows 上的控制命令偶尔会先超时、随后才开始播放。0.6.1 仍由官方 CLI 完成鉴权、搜索、个人歌单读取、取流和歌单队列；暂停、继续、停止、音量、播放进度和实时频段连接 CLI 创建的 `ncm-mpv` 本地管道。插件会在媒体路径变化以及播放/切歌后用官方 `ncm-cli state` 核对真实曲目，避免不可播歌曲被 CLI 跳过后界面仍显示原请求名称。工具栏弹窗和网页悬浮窗统一经过后台路由：数据加载、播放控制和实时频段各使用独立的 Native Messaging 长连接，读取大歌单时不会再把频谱与控制请求堵在同一队列中，同时两处界面共享后台播放事务锁。扩展不会读取或接收播放 URL。
...
网易云播放期间，只要播放器面板或底部频谱坞可见，就会临时在 mpv 内启用固定的 10 频段分析器（31Hz–16kHz）。视觉映射同时使用绝对响度、相邻频段差值和相邻帧瞬态，并对差值做平方增强，减少大段频谱柱挤在相同高度的情况；两者都关闭、暂停或切换 QQ 音乐后会移除分析器。当前版本选择只分析本扩展正在播放的音乐，不申请桌面录音权限、不监听其他应用或整台电脑的声音，也不读取播放 URL、上传或落盘；旧版 mpv 不支持时自动退回播放状态动画。

搜索结果按“平台 + 歌曲/歌单”分别缓存，切换分类后关键词、列表和滚动位置都会保留。网易云下的“我的音乐”可通过官方 ncm-cli 读取喜欢歌曲、自己创建的歌单和收藏的歌单，歌曲首批按 40 首加载，同一歌单的重复读取会合并，列表一次性批量挂载后再按需继续加载；个人列表只保存在当前页面内存中，刷新后重新从官方账号读取。授权按网站保存，扩展不会读取网页正文；Edge 设置页、扩展页等内部页面不能注入。
...
桌面会生成 `CloudMusicEdge-v0.6.1.zip`。压缩包不包含 QQ Key、网易云凭据、个人歌单缓存、外观设置或登录态。另一台 Windows 电脑解压后重新运行 `安装.cmd`，再分别完成账号授权即可。
