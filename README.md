...
`ncm-cli 0.1.7` 在 Windows 上的控制命令偶尔会先超时、随后才开始播放。0.5.0 仍由官方 CLI 完成鉴权、搜索、个人歌单读取、取流和歌单队列，但暂停、继续、停止、音量、播放进度和实时频段只连接 CLI 创建的 `ncm-mpv` 本地管道，避免播放已经出声后控制状态丢失。扩展不会读取或接收播放 URL。

### 网页悬浮播放器

在普通网页中打开扩展，点击“在当前网站启用悬浮球”。44px 悬浮球可以拖动并自动吸附左右边缘；开始拖动时会收起播放器。单击会展开上一首、播放/暂停、下一首快捷操作，再点击末端按钮可打开紧凑的双栏播放器。播放器优先显示在悬浮球上方，空间不足时自动改到下方；左栏是播放和音频视觉控件，右栏是可独立滚动的搜索、个人音乐与歌单歌曲列表。

点击歌单会先进入歌单详情，再由用户选择歌曲播放；详情、搜索分类和个人音乐会分别保留列表、分页与滚动位置。右上角调色盘可选择曜石、雾蓝、墨绿、暮色主题，并调整面板透明度、字体风格、文字大小，以及是否在网页底部显示频谱坞。底部频谱坞居中占约 44% 视口宽度，高度可在 72–180px 间调节，并提供当前曲目和上一首、播放/暂停、下一首控制；所有外观设置都保存在扩展本地存储中。
...
桌面会生成 `CloudMusicEdge-v0.5.0.zip`。压缩包不包含 QQ Key、网易云凭据、个人歌单缓存、外观设置或登录态。另一台 Windows 电脑解压后重新运行 `安装.cmd`，再分别完成账号授权即可。
