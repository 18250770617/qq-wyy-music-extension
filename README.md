...
- 点击可播歌曲播放；不可播、会员受限、数字专辑和试听歌曲也会保留在列表中并标明状态。
...
`ncm-cli 0.1.7` 在 Windows 上的控制命令偶尔会先超时、随后才开始播放。0.6.0 仍由官方 CLI 完成鉴权、搜索、个人歌单读取、取流和歌单队列；暂停、继续、停止、音量、播放进度和实时频段连接 CLI 创建的 `ncm-mpv` 本地管道。插件会在媒体路径变化以及播放/切歌后用官方 `ncm-cli state` 核对真实曲目，避免不可播歌曲被 CLI 跳过后界面仍显示原请求名称。扩展不会读取或接收播放 URL。
...
点击歌单会先进入歌单详情，再由用户选择歌曲播放；详情、搜索分类和个人音乐会分别保留列表、分页与滚动位置。歌曲列表不会隐藏不可播项：完整播放、试听、当前端无版权、需要音乐会员、数字专辑和未知状态都会保留原顺序，只有明确可播或可试听的项目能够点击。右上角调色盘可选择曜石、雾蓝、墨绿、暮色主题，并调整背景不透明度（32%–100%）、字体风格、文字大小，以及是否在网页底部显示频谱坞。底部频谱坞无整块背景，居中占约 62% 视口宽度，高度可在 72–180px 间调节；频谱从页面底边单向向上生长，上一首、播放/暂停、下一首按钮也没有容器底色。所有外观设置都保存在扩展本地存储中。

网易云播放期间，只要播放器面板或底部频谱坞可见，就会临时在 mpv 内启用固定的 10 频段分析器（31Hz–16kHz），视觉柱按各频段 RMS 能量实时变化；两者都关闭、暂停或切换 QQ 音乐后会移除分析器。当前版本选择只分析本扩展正在播放的音乐，不申请桌面录音权限、不监听其他应用或整台电脑的声音，也不读取播放 URL、上传或落盘；旧版 mpv 不支持时自动退回播放状态动画。
...
桌面会生成 `CloudMusicEdge-v0.6.0.zip`。压缩包不包含 QQ Key、网易云凭据、个人歌单缓存、外观设置或登录态。另一台 Windows 电脑解压后重新运行 `安装.cmd`，再分别完成账号授权即可。
