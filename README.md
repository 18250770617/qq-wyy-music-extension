...
`ncm-cli 0.1.7` 在 Windows 上的控制命令偶尔会先超时、随后才开始播放。0.6.2 仍由官方 CLI 完成鉴权、搜索、个人歌单读取、取流和歌单队列；暂停、继续、停止、音量、播放进度和实时频段连接 CLI 创建的 `ncm-mpv` 本地管道。插件会在媒体路径变化以及播放/切歌后用官方 `ncm-cli state` 核对真实曲目，避免不可播歌曲被 CLI 跳过后界面仍显示原请求名称。工具栏弹窗和网页悬浮窗统一经过后台路由：数据加载、播放控制和实时频段各使用独立的 Native Messaging 长连接，读取大歌单时不会再把频谱与控制请求堵在同一队列中，同时两处界面共享后台播放事务锁。扩展不会读取或接收播放 URL。
...
桌面会生成 `CloudMusicEdge-v0.6.2.zip`。压缩包不包含 QQ Key、网易云凭据、个人歌单缓存、外观设置或登录态。另一台 Windows 电脑解压后重新运行 `安装.cmd`，再分别完成账号授权即可。
