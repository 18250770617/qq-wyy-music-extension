...
4. 页面右侧出现圆形音频悬浮球。拖动后松手会自动吸附到最近边缘；单击展开快捷按钮，点击末端按钮打开完整播放器。
...
`ncm-cli 0.1.7` 在 Windows 上的控制命令偶尔会先超时、随后才开始播放。0.4.0 仍由官方 CLI 完成鉴权、搜索、个人歌单读取、取流和歌单队列，但暂停、继续、停止、音量和播放进度会直接连接 CLI 创建的 `ncm-mpv` 本地管道，避免播放已经出声后控制状态丢失。扩展不会读取或接收播放 URL。

### 网页悬浮播放器

在普通网页中打开扩展，点击“在当前网站启用悬浮球”。48px 悬浮球可以拖动并自动吸附左右边缘；单击会展开上一首、播放/暂停、下一首快捷操作，再点击末端按钮可打开完整播放器。完整播放器包含真实播放进度、图形化波形/能量柱、唱片动画、进度拖动、音量以及歌曲/歌单搜索。视觉波形由播放状态驱动，不采集系统音频。

搜索结果按“平台 + 歌曲/歌单”分别缓存，切换分类后关键词、列表和滚动位置都会保留。网易云下的“我的音乐”可通过官方 ncm-cli 读取喜欢歌曲、自己创建的歌单和收藏的歌单，长列表可按需继续加载；个人列表只保存在当前页面内存中，刷新后重新从官方账号读取。授权按网站保存，扩展不会读取网页正文；Edge 设置页、扩展页等内部页面不能注入。
...
桌面会生成 `CloudMusicEdge-v0.4.0.zip`。压缩包不包含 QQ Key、网易云凭据、个人歌单缓存或登录态。另一台 Windows 电脑解压后重新运行 `安装.cmd`，再分别完成账号授权即可。
