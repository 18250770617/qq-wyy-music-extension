...
try {
    ...
    @('AGENTS.md','README.md','启动助手.cmd','安装.cmd','检测连接.cmd','卸载.cmd','extension','native-host','tools','docs','tests') | ForEach-Object {
    ...
    Get-ChildItem -LiteralPath $packageRoot -Recurse -File | Where-Object { $_.Name -match 'qq\.key|settings\.json|\.pem$|\.pfx$' } | ForEach-Object { throw "打包已中止：发现疑似凭据文件 $($_.FullName)" }
